...
AP.add_argument("--dali_radius", required=False, type=float, default=None, help="DALI inclusion radius (default none)")
AP.add_argument("--dists", required=False, default="0.5,1,2,4", help="LDDT distance thresholds, comma-separated (default 0.5,1,2,4)")
AP.add_argument("--symmetry", required=False, choices=[ "both", "either" ], default="first", help="Set R0/dali_radius according to both / either / first (default first)")
...
def dali_score(pos1s, x1s, y1s, z1s, pos2s, x2s, y2s, z2s):
	...
	for coli in range(n):
		...
		for colj in range(0, n):
			...
			if symmetry == "first":
				if not DALI_radius is None and (dij1 > DALI_radius):
					continue
			elif symmetry == "both":
				if not DALI_radius is None and (dij1 > DALI_radius and dij2 > DALI_radius):
					continue
			elif symmetry == "either":
				if not DALI_radius is None and (dij1 > DALI_radius or dij2 > DALI_radius):
					continue
...
def lddt_score(pos1s, x1s, y1s, z1s, pos2s, x2s, y2s, z2s):
	...
	for t in thresholds: # [ 0.5, 1, 2, 4 ]:
		score = lddt_score_t(pos1s, pos2s, D1, D2, t)
		total += score
	avg = total/4
	return avg

def lddt_score_fm_pairwise(pos1s, x1s, y1s, z1s, pos2s, x2s, y2s, z2s):
	nr_cols = len(pos1s)
	assert len(pos2s) == nr_cols
	...
	for i in range(nr_cols):
		...
		for j in range(i+1, nr_cols):
			...
			if symmetry == "first":
				if d1 > R0:
					print("pos1i=%d pos1j=%d d1=%.3g include=F" % (pos1i, pos1j, d1))
					continue
			elif symmetry == "both":
				if d1 > R0 and d2 > R0:
					continue
			...
			nr_considered += 1
			d_l = abs(d1 - d2)
			score = 0.25 * (int(d_l < 0.5) + int(d_l < 1.0) + int(d_l < 2.0) + int(d_l < 4.0))
			total_score += score
			s = "pos1i=%d pos1j=%d d1=%.3g include=T" % (pos1i, pos1j, d1)
			s += " d_l=%.3g score=%.4f" % (d_l, score)
			print(s)
	return total_score/nr_considered
